// Upstream: spec-0004@v3 INV8, INV9, INV13, INV17, INV19; S5, S6, S13, S17.
...
test('Claude and Codex setup in either order share one floor and are idempotent', async () => {
  for (const invocations of [
    ...
    assert.match(snapshot.claude, /full dispatcher and interactive shaper/i);
    assert.match(snapshot.codex, /full dispatcher and interactive shaper/i);
