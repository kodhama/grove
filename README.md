...
Grove charters ten agent roles across the stages of the pipeline —
...
artifact record itself, plus one advisory pre-execution planner — fourteen
roles in all. Every role except the dispatcher
...
| implementation-planner | pre-4 | read-only reconnaissance and advisory red → green → refactor planning for one ratified code-bearing specification | yes |
...
|---|---|---|---|---|---|
| `claude-interactive` | Yes | Not claimed | Unsupported | host-native | The package has an established interactive load path, but Grove does not claim 0.5.0 support until the complete release record passes. |
