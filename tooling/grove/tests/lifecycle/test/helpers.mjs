...
export async function fixture() {
  ...
  await mkdir(join(packageRoot, 'runtime', 'gates', 'lib'), { recursive: true });
  await mkdir(join(packageRoot, 'runtime', 'gates', 'bin'), { recursive: true });
  await mkdir(join(packageRoot, 'reference', 'gates'), { recursive: true });
  await mkdir(join(packageRoot, 'metadata'), { recursive: true });
  await mkdir(repoRoot, { recursive: true });

  await writeFile(join(packageRoot, 'VERSION'), '0.3.0\n');
  await writeFile(join(packageRoot, 'runtime', 'gates', 'lib', 'profile.mjs'), 'export const ok = true;\n');
  await writeFile(join(packageRoot, 'runtime', 'gates', 'bin', 'resolve-profile.mjs'), '#!/usr/bin/env node\n');
  ...
  await writeFile(
    join(packageRoot, 'metadata', 'config-tokens.json'),
    JSON.stringify({
      schema_version: 1,
  ...
  await writeFile(
    join(packageRoot, 'metadata', 'surfaces.json'),
    JSON.stringify({
      schema_version: 1,
      version: '0.3.0',
      rows: [
  ...
  await writeFile(
    join(packageRoot, 'metadata', 'codex-launchers.json'),
    JSON.stringify({
      generated: true,
      source: 'roles.json',
      launchers: [
