...
async function validateLaunchers() {
  ...
  const generated = JSON.parse(await readFile(
    join(paths.package_snapshot, 'metadata', 'codex-launchers.json'),
    'utf8',
  ));
...
async function validatePreparedBundle() {
  ...
  if (!Array.isArray(manifest.harness?.files) || manifest.harness.files.length < 3) {
    throw new Error('probe manifest lacks the complete hash-bound harness');
  }
  const declaredHarness = new Map(
    manifest.harness.files.map((file) => [file.path, file.sha256]),
  );
  for (const required of [
    'run-probe.mjs',
    manifest.harness.probe_module,
    manifest.harness.release_module,
  ]) {
    if (!/^[0-9a-f]{64}$/.test(declaredHarness.get(required) ?? '')) {
      throw new Error(`probe harness does not hash-bind ${required}`);
    }
  }
  for (const [path, expectedDigest] of declaredHarness) {
    const actualDigest = await fileSha256(bundlePath(path));
    if (actualDigest !== expectedDigest) {
      throw new Error(`probe harness digest differs at ${path}`);
    }
  }
  const [probeModule, releaseModule] = await Promise.all([
    import(pathToFileURL(bundlePath(manifest.harness.probe_module)).href),
    import(pathToFileURL(bundlePath(manifest.harness.release_module)).href),
  ]);
  if (
    typeof probeModule.parseCodexSessionEvidenceJsonl !== 'function'
    || typeof probeModule.validateCodexV2SessionEvidence !== 'function'
    || typeof releaseModule.validateSupportRecord !== 'function'
    || typeof releaseModule.validatePackageTree !== 'function'
    || typeof releaseModule.packageTreeDigest !== 'function'
  ) {
    throw new Error('probe harness modules do not expose the required validators');
  }
  const observedPackageDigest = await releaseModule.packageTreeDigest(paths.package_snapshot);
  ...
  const observedCompositionDigest = await releaseModule.packageTreeDigest(paths.composition_fixture);
  if (
    manifest.composition_fixture?.path !== FIXED_PATHS.composition_fixture
    || manifest.composition_fixture?.exact_package_tree_sha256 !== manifest.package_tree_sha256
    || observedCompositionDigest !== manifest.composition_fixture?.fixture_tree_sha256
  ) {
    throw new Error('composition-only fixture differs from its hash-bound declaration');
  }
  const exactSurfaces = JSON.parse(await readFile(
    join(paths.package_snapshot, 'metadata', 'surfaces.json'),
    'utf8',
  ));
  const fixtureSurfaces = JSON.parse(await readFile(
    join(paths.composition_fixture, 'metadata', 'surfaces.json'),
    'utf8',
  ));
  const exactRow = exactSurfaces.rows.find(
    (row) => row.surface_id === manifest.composition_fixture.surface_id,
  );
  const fixtureRow = fixtureSurfaces.rows.find(
    (row) => row.surface_id === manifest.composition_fixture.surface_id,
  );
  if (
    exactRow?.release_state !== 'candidate'
    || fixtureRow?.release_state !== manifest.composition_fixture.release_state_override
  ) {
    throw new Error('composition fixture does not preserve the exact candidate/support seam');
  }
  const packageValidation = await releaseModule.validatePackageTree(paths.package_snapshot);
  if (!packageValidation.ok) {
    throw new Error(`exact package snapshot failed preflight: ${packageValidation.errors.join('; ')}`);
  }
  return {
...
async function main() {
  ...
  if (command != null && command !== '--preflight') throw new Error(`unknown argument: ${command}`);
  const prepared = await validatePreparedBundle();
  if (command === '--preflight') {
    process.stdout.write('Grove probe preflight passed: exact snapshot, launchers, composition seam, and harness digests are valid.\n');
    return;
  }
  ...
  const phaseReports = [];
  try {
    const { observedPackageDigest, probeModule, releaseModule } = prepared;
    ...
    const surfaces = JSON.parse(await readFile(join(packageRoot, 'metadata', 'surfaces.json'), 'utf8'));
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        phases: phaseReports,
        note: 'Review raw JSONL before promoting metadata/surfaces.json; this runner never edits the matrix.',
      }, null, 2)}\n`,
    );
    assertNotInterrupted();
    process.stdout.write(
