---
id: adr-0038-transient-specs-frozen-at-fulfillment
type: adr
...
informed_by: [adr-0031-multi-host-distribution, adr-0035-plugin-and-consumer-boundary]
owner: agent
updated: 2026-07-25
---

# ADR-0038 (draft): specs as transient gated contracts, frozen at fulfillment

> Renumber note (2026-07-25): this draft was authored 2026-07-13 as
> `adr-0011`, then moved to `adr-0012` on 2026-07-14 when the approved
> `adr-0011-relations-companion` took that id. While this validation-gated
> branch remained parked, Grove main approved a different
> `adr-0012-methodology-delivery-machinery`, then approved ADRs 0033–0035;
> open PR #140 reserved ADR-0036, and concurrent PR #141 opened with a
> colliding old id, making ADR-0037 unsafe to publish without coordinating
> that repair. This draft yielded the slot and was ported onto current main
> as `adr-0038`;
> the paired annex moved `research-0011` → `research-0012` →
> `research-0038`. The proposed lifecycle thesis and deliberate `draft`
> status are unchanged; rollout wording was reconciled with ADRs 0031 and
> 0035's current canonical-corpus and generated-adapter boundary.
...
> development. Evidence base: `research-0038-spec-lifecycle-prior-art`
...
- **While `draft`: nothing changes anywhere.** adr-0004 governs; no
  canonical charter, generated adapter/reference projection, consumer
  contract, or README moves. This ADR may not be cited as `depends_on`
  by any building artifact (lifecycle rule).
- **If approved, the rollout owes, in order**: (1) an upstream partial
  ...
  *at trellis level* — grove cannot contradict its dependency; this is
  ...
  pointer; (4) canonical charter updates (contract-author, executor,
  conformance-reviewer: binding gate + held-out probes), followed by
  deterministic regeneration of the plugin reference corpus and both
  host adapters plus release-drift validation; (5) the durability sensor
  ships report-only from the canonical corpus-reviewer surface, with its
  generated projections validated — the only piece that may ship
  **before** approval, being model-agnostic measurement. No rollout
  reintroduces runtime material under the consumer `.grove/` contract.
...
- Frontmatter present, well-typed; `depends_on` resolve (`adr-0004`
  ...
  `draft`). The non-flow `informed_by` references also resolve
  (`adr-0031`, `adr-0035`, both approved) and constrain only future
  propagation mechanics, not the lifecycle thesis. PASS.
